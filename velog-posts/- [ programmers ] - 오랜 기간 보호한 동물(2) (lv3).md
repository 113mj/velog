...
<p>먼저, 입소일과 입양일을 기준으로 보호 기간을 계산하기 위해 두 테이블을 <code>JOIN</code>합니다.
오래 보호된 동물을 찾기 위해 보호 시작일에서 입양일을 뺀 값을 계산하며, 이 값이 클수록 더 오래 보호된 동물입니다.
이를 <code>temp</code>라는 컬럼으로 정의한 후, 보호 기간을 기준으로 내림차순 정렬하고 <code>LIMIT</code>을 사용하여 가장 오래 보호된 2마리의 동물만 조회합니다.</p>
<h3 id="정답">정답</h3>
<pre><code class="language-sql">with sub_date as(
SELECT
    i.ANIMAL_ID,
    i.NAME,
